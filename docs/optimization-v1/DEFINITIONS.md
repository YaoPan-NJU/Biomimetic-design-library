...
7. canon 唯一真源 = `prototypes_db/*.json`（已冻结，只在其上直接编辑）。`prototype.md` 只从 canon 生成（`generate_prototype_md.py`），不手改。⚠️**严禁运行 `build_prototypes_db.py`**：它从 `extraction/` 原始提取 JSON 反向重建 canon，只兜回 enrichment，会冲掉 Phase 2/3/4 的手工清理、chimera 复活。
...
**粒度（Phase 5 定）**：每个 active 原型建 **1–几张核心可迁移原理卡**即可，碎片/实例机制作为证据保留、不必每条建卡。**未建成合格卡的机制不保留空 `causal_chain` 骨架**（删掉整个键），以免"空 basis 要素"造成假绿。保留 `causal_chain` 的机制必须是合格卡，且每个要素（含 boundary）都有 `basis`。
