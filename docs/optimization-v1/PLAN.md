...
7. **canon 唯一真源是 `prototypes_db/*.json`（已冻结）。** `prototypes/*/prototype.md` 是派生物，只能**从 canon 生成**（`generate_prototype_md.py`），不手改。⚠️**严禁运行 `tools/build_prototypes_db.py` 来"重建" canon**——它是从 `extraction/` 的 311 个原始提取 JSON 反向生成 canon、只兜回 enrichment 字段，会把 Phase 2/3/4 的手工清理（去 chimera、降级移动、pollutant 回填）整体冲掉、chimera 复活。canon 自此只允许在其上直接编辑，不允许从提取源重建。
...
**步骤**
1. 生成派生物：**不要运行 `build_prototypes_db.py`（见铁律 7，会冲掉清理）**。只从冻结的 canon 生成 `prototype.md`（`python tools/generate_prototype_md.py`），随后跑全套检查。
