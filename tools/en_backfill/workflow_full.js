export const meta = {
  name: 'bmdl-en-full',
  description: 'Translate all remaining prototypes to /tmp parts (one agent per prototype)',
  phases: [{ title: 'Translate', detail: 'agents, one per prototype' }],
}
...
- 专有名词原样保留：PDB ID（如 9U5A）、基因名（如 SLC22A11）、物种学名（如 Homo sapiens）、化学式、数字、单位、文献引用（如 Cha 2000）、缩写（如 E1S、PFAS、Km）。
- 中文人名/文献引用不要在英文里。
...
phase('Translate')
const results = await parallel(args.ids.map(id => () =>
  agent(
    `${SPEC}\n\n【本次待翻译原型】\n- proto_id: ${id}\n- 源文件路径: /tmp/bmdl_en_backfill/sources/${id}.json\n- 输出目录: /tmp/bmdl_en_backfill/parts/${id}\n\n请先 Read 源文件，核对 proto_id 与 mechanisms 数量，翻译全部机制后分 part 写入，最后返回统计 JSON。`,
    { label: `translate:${id}`, phase: 'Translate', schema: RESULT_SCHEMA }
  )
))
